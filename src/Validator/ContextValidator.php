...
class ContextValidator extends AbstractValidator
{
    ...
    public function validate(EntityInterface $entity)
    {
        if (!$entity instanceof Context) {
            throw new Exception('The Entity to validate must be an instance of \Fei\Service\Filer\Entity\Context');
        }
    ...
    {
        if (empty($value) && ((!is_string($value) && $value !== 0) || (is_string($value) && $value === ''))) {
